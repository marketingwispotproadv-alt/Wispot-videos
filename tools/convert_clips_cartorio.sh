#!/bin/bash
# Converte os takes do vídeo de cartório para o formato do projeto.
#
#     tools/convert_clips_cartorio.sh
#
# Lê o mapa `tools/cartorio-mapa.txt` (nome de destino, nome de origem) e
# escreve em `public/cartorio/clips/`. A origem fica em `raw/cartorio/`.
#
...
#
# O vídeo é reencodado, não copiado: os arquivos chegaram 1024x576 com
# `rotation -90` nos metadados. Reencodar deixa o autorotate gravar a
# orientação no quadro — 576x1024 de verdade — e tira o risco de o render sair
# deitado. O upscale para 1080x1920 é lanczos aqui em vez de ficar para o
# Chromium, que ainda por cima teria de esticar 2,5x por causa do salto de
...
") || { echo "MEDICAO FALHOU: $novo"; continue; }
  npx --no-install remotion ffmpeg -nostdin -v error -y -i "$src" \
    ...
    -ar 48000 -c:a aac -b:a 192k "public/cartorio/clips/$novo.mp4" 2>&1 | grep -v Warning
  echo "OK $novo  ($med)"
done < tools/cartorio-mapa.txt
echo "FIM: $(ls public/cartorio/clips/*.mp4 2>/dev/null | wc -l) de 21"
